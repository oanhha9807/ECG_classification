...
class EEGViT_pretrained(nn.Module):
    def __init__(self):
        ...
        model.classifier = torch.nn.Sequential(torch.nn.Linear(768, 5, ))
...
class RNN_att(nn.Module):
    ...
    def forward(self, X, spectrogram_instance, training):
        ...
        out_1 = torch.sigmoid(resnet_feature)
        out_2 = torch.sigmoid(logits)

        out_final = torch.max(out_1, out_2)

        return resnet_feature, logits, out_final
...
def train_batch(X, image, y, model, optimizer,criterion, gpu_id=None, **kwargs):
    ...
    optimizer.zero_grad()
    f1, f2, _max_proba = model(X, spectrogram_instance, training=True)

    # loss1 = loss1(S, A)

    loss = criterion(f1, y) + criterion(f2, y)
...
def predict(model, X, y, thr):
    ...
    _, _, _max_proba = model(x_batch, y_batch, training = False)
    # logits_ = model(X, y)  # (batch_size, n_classes)
    # probabilities = torch.sigmoid(logits_).cpu()
    probabilities = _max_proba.cpu()
...
def compute_loss(model, dataloader,criterion, gpu_id=None):
    ...
    with torch.no_grad():
        ...
        for i, (x_batch,image, y_batchs) in tqdm(enumerate(dataloader)):
            ...
            f1, f2, _max_proba = model(X, spectrogram_instance,  training=False)
            loss = criterion(f1, y_batch) + criterion(f2, y_batch)
...
def threshold_optimization(model, dataloader, gpu_id=None):
    ...
    with torch.no_grad():
        ...
        for _, (Xs,image, Ys) in tqdm(enumerate(dataloader)):
            ...
            _, _, _max_proba = model(X, spectrogram_instance, training=False)
...
def main():
    ...
    parser.add_argument('-learning_rate', type=float, default=0.01)#best 0.003
    ...
    image = torch.randn(1, 36, 256, 256).cuda()   # Example input
    training = False
    ...
    for ii in epochs:
        ...
        if val_loss == np.min(valid_mean_losses):
            ...
            print()
        torch.save(model.state_dict(), 'weights/model_late.pt')
        ...
        acc_train = (np.mean(matrix_train[:, 0]) + np.mean(matrix_train[:, 3])) / (
                np.mean(matrix_train[:, 0]) + np.mean(matrix_train[:, 1]) + np.mean(matrix_train[:, 2]) + np.mean(matrix_train[:, 3]))
        ...
        save_results_to_txt(ii, f1,max_acc, matrix, 'combination_2_model.txt')
