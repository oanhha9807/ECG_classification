...
class RNN_att(nn.Module):
    ...
    def forward(self, spectrogram_instance):

        resnet_feature = self.model1(spectrogram_instance)

        # logits = resnet_feature.view(resnet_feature.size(0), -1)
        # # Multi-label predictions
        #
        logits = self.fc(resnet_feature)
...
def train_batch(X, image, y, model, optimizer,criterion, gpu_id=None, **kwargs):
    ...
    optimizer.zero_grad()
    out = model(spectrogram_instance)
...
def compute_loss(model, dataloader,criterion, gpu_id=None):
    ...
    with torch.no_grad():
        ...
        for i, (x_batch,image, y_batchs) in tqdm(enumerate(dataloader)):
            ...
            y_pred = model(spectrogram_instance)
...
def threshold_optimization(model, dataloader, gpu_id=None):
    ...
    with torch.no_grad():
        ...
        for _, (Xs,image, Ys) in tqdm(enumerate(dataloader)):
            ...
            logits_ = model(spectrogram_instance)
...
def main():
    ...
    parser.add_argument('-data', default='/data/oanh/ECG_official/RNN_12/',
                        help="Path to the dataset.")

    # parser.add_argument('-data_image', default='/data/oanh/Bio/Images/',
    parser.add_argument('-data_image', default='/data/oanh/Bio/Images/',
    # parser.add_argument('-data_image', default='/data/oanh/ECG_official/Images/',
                        help="Path to the dataset.")
    ...
    for ii in epochs:
        ...
        model.eval()
        torch.save(model.state_dict(), 'weights/model_only_ViT.pt')
        ...
        save_results_to_txt(ii, f1,max_acc, matrix, 'only_viT.txt')
