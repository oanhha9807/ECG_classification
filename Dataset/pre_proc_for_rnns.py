...
def X_for_RNNs_old(path, partition='train', save_dir=None):

	file = str(path) + '/X_' + str(partition) + '_processed_1.pickle'
...
def X_for_RNNs(path, partition='train', save_dir=None):
	...
	for i in range(np.shape(X)[0]):
		...
		lead_V2 = X[i][:, 7]
		for j in range(12):
			lead = X[i][:, j]  # X[i]: (1000, 12)

			# apply a band pass filter (0.05, 40hz)
			lead_I = sosfilt(band_pass_filter, lead)
			# lead_II = sosfilt(band_pass_filter, lead_II)
			# lead_V2 = sosfilt(band_pass_filter, lead_V2)
...
def labels(path, partition='train', save_dir=None):
	...
	file = str(path) + '/y_' + str(partition) + '_processed_1.pickle'
...
save_dir = '/data/oanh/Bio/new_data_2/'
X_for_RNNs_old(path, partition='train', save_dir=save_dir)
labels(path, partition='train', save_dir=save_dir)

X_for_RNNs_old(path, partition='dev', save_dir=save_dir)
labels(path, partition='dev', save_dir=save_dir)

X_for_RNNs_old(path, partition='test', save_dir=save_dir)
labels(path, partition='test', save_dir=save_dir)